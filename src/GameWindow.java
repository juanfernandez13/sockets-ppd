...
public class GameWindow extends JFrame {
    ...
    // Status bar colors por estado
    private static final Color ST_MY_TURN   = new Color(35, 130,  70);
    private static final Color ST_OPP_TURN  = new Color(45,  65, 115);
    private static final Color ST_CAPTURE   = new Color(170,  45,  45);
    private static final Color ST_OPP_CAP   = new Color(160,  95,  20);
    private static final Color ST_GAMEOVER  = new Color(55,  55,  65);
    private static final Color ST_WAITING   = new Color(50,  55,  75);
     ...
     * Vincula ao GameWindow, inicia o loop de recepção e envia INIT se servidor.
     */
    public GameWindow(Connection conn, boolean isServer, String name) {
        ...
        // Vincula e começa a receber mensagens
        connection.startGame(this);
        onConnected();   // chamado diretamente no EDT — conexão já está pronta
    ...
    // ── Painel com os dois cartões de jogador ─────────────────────────────
    ...
    private JPanel buildPlayerCard(int playerNum) {
        ...
        // Círculo colorido (bolinha da peça)
    ...
    public void onConnected() {
        // Chamado no EDT diretamente do construtor — sem invokeLater necessário
        if (isServer) {
            myPlayer = 1;
            connection.send("INIT:" + myName);
            setTitle("Dara  |  Jogador 1  —  " + myName);
            refreshAll();
        }
        // cliente aguarda mensagem INIT
    }
    ...
    public void onDisconnected() {
        SwingUtilities.invokeLater(() -> {
            if (logic.getPhase() != GameLogic.Phase.GAMEOVER) {
                setStatus(ST_GAMEOVER, "Conexão encerrada.");
                JOptionPane.showMessageDialog(this, "Oponente desconectado.",
    ...
    public void onBoardClick(int row, int col) {
        ...
        if (logic.getPhase() == GameLogic.Phase.GAMEOVER) return;
        if (logic.getCurrentPlayer() != myPlayer) return;
    ...
    private void refreshAll() {
        ...
        // Badge — texto "SUA VEZ" no próprio cartão, "VEZ DELE" no adversário
        ...
        // Destaque visual do cartão ativo
        ...
        // Barra de status com cor e mensagem claras
        if (myPlayer == 0) return;
        Color bg;
        String txt;
